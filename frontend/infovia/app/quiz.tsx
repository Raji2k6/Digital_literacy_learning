...
const fallbackQuestions: QuizQuestion[] = [
  {
    ...
    correctAnswer: 1,
    explanation: "MIL helps people evaluate and interpret information critically."
  },
  {
    ...
    correctAnswer: 0,
    explanation: "Misinformation is false information shared without harmful intent."
  }
  // 👉 You can add more fallback questions here
]
    ...
    try {
      ...
      for (let i = 0; i < NUM_QUESTIONS; i++) {
        try {
          ...
          let questionData: QuizQuestion | null = null
          ...
          if (responseData.questions && Array.isArray(responseData.questions) && responseData.questions.length > 0) {
            ...
            questionData = responseData as QuizQuestion
          }
          ...
          if (
            ...
            console.warn(`⚠ Invalid question structure for question ${i + 1}`, responseData)
          }
  ...
  if (loading) {
    return (
      <LinearGradient colors={['#6a11cb', '#2575fc']} style={styles.container}>
        <Text style={styles.loadingText}>Generating your personalised quiz...</Text>
        <Text style={styles.loadingSubtext}>Please wait while we create your questions.</Text>
      </LinearGradient>
    )
  }
    ...
    <LinearGradient colors={['#6a11cb', '#2575fc']} style={styles.container}>
      <View style={styles.card}>
        ...
        <TouchableOpacity
          ...
          disabled={selectedAnswer === null}
        >
          <Text style={styles.nextText}>{currentQuestionIndex + 1 === questions.length ? 'Complete Quiz' : 'Next Question'}</Text>
        </TouchableOpacity>
      </View>
    </LinearGradient>
  )
...
const styles = StyleSheet.create({
  ...
  card: { width: '90%', backgroundColor: '#fff', borderRadius: 20, padding: 25, shadowColor: '#000', shadowOffset: { width: 0, height: 10 }, shadowOpacity: 0.3, shadowRadius: 20, elevation: 10 },
  question: { fontSize: 22, fontWeight: '700', marginBottom: 20, color: '#333' },
  option: { backgroundColor: '#e6f0ff', padding: 16, borderRadius: 15, marginBottom: 12, alignItems: 'center', shadowColor: '#6a11cb', shadowOffset: { width: 0, height: 5 }, shadowOpacity: 0.2, shadowRadius: 10, elevation: 3 },
  ...
  next: { backgroundColor: '#6a11cb', padding: 16, borderRadius: 15, alignItems: 'center', marginTop: 20, shadowColor: '#6a11cb', shadowOffset: { width: 0, height: 5 }, shadowOpacity: 0.3, shadowRadius: 15, elevation: 5 },
  ...
  loadingText: { fontSize: 20, fontWeight: '700', textAlign: 'center', color: '#333', marginBottom: 10 },
  loadingSubtext: { fontSize: 16, textAlign: 'center', color: '#666' },
  completedTitle: { fontSize: 24, fontWeight: '700', textAlign: 'center', color: '#6a11cb', marginBottom: 20 },
  scoreText: { fontSize: 20, fontWeight: '600', textAlign: 'center', color: '#333', marginBottom: 10 },
  percentageText: { fontSize: 18, textAlign: 'center', color: '#2575fc', marginBottom: 20 },
  errorText: { fontSize: 18, textAlign: 'center', color: '#ff4444', marginBottom: 20 },
  explanation: { marginTop: 15, fontSize: 16, fontWeight: '500', textAlign: 'center' },
